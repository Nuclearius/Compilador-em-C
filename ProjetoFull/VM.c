...
static void click_open (GtkButton *btn, gpointer used_data){
 ...
 static void app_activate (GApplication *app, gpointer user_data) {
    ...
    btnrs = gtk_button_new_with_label ("Run SbS");
	...
	gtk_box_append (GTK_BOX (boxh), btnr);
	gtk_box_append (GTK_BOX (boxh), dmy30);
	gtk_box_append (GTK_BOX (boxh), btnrs);
...
///entrada
    ...
    g_signal_connect(G_OBJECT(entry),"activate", G_CALLBACK(on_entry_activate),tb3);/* when you press ENTER */
...
void execucaoVM(bool step, gpointer output)
{   int I =0;
    int s = -1;
...
int lerLinha(char *line, gpointer output)
{
	...
	gtk_text_buffer_insert(GTK_TEXT_BUFFER (output),&end, s,-1);
		...
		Memoria[p-1]= Memoria[p-1]-Memoria[p];		//subtra\E7ao
		p--;
		l++;
	}
	 else if (!strcmp(comando,"MULT")){
		Memoria[p-1]= Memoria[p-1]*Memoria[p];		//multiplica\E7ao
		p--;
		l++;
	}
